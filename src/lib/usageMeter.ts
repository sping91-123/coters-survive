...
export const usageBuckets: UsageBucket[] = [
  ...
  },
  {
    ...
    label: "글로벌 레이더",
    shortLabel: "글로벌",
    description: "미국주식, ETF, 해외선물 흐름을 다시 확인합니다.",
    freeDailyLimit: 1,
    proDailyLimit: 100
  },
  {
    ...
    label: "글로벌 AI 브리핑",
    shortLabel: "글로벌 AI",
    description: "글로벌 뉴스와 매크로 흐름을 한 번에 정리합니다.",
    freeDailyLimit: 1,
    proDailyLimit: 30
  },
  {
  ...
  },
  {
    ...
    label: "글로벌 알림 설정",
    shortLabel: "글로벌 알림",
    description: "글로벌 레이더 알림 조건을 만들고 다시 확인합니다.",
    freeDailyLimit: 1,
    proDailyLimit: 20
  }
];
...
export function getUsageGate(bucketId: UsageBucketId, isPaid: boolean) {
  ...
  return {
    ...
    message: allowed
      ? ""
      : isPaid
        ? `오늘 ${state.label} Pro 확인 한도를 모두 사용했습니다. 내일 다시 열립니다.`
        : `오늘 Basic ${state.label} 확인 한도를 모두 사용했습니다. Pro에서는 장중 재확인이 가능합니다.`
  };
}
